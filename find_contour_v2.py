...
def draw_contours_A(file_name, img):
    ...
    cv2.imwrite(now + '_1_' + file_name + '_hsv_A.png', hsv)

    # ガウス変換
    gauss = cv2.GaussianBlur(hsv,(9, 9),3)
    cv2.imwrite(now + '_2_' + file_name + '_gauss_A.png', gauss)

    # 色調分割
    img_H, img_S, img_V = cv2.split(gauss)
    cv2.imwrite(now + '_3_' + file_name + '_H_of_HSV_A.png', img_H)
    _thre, img_mask = cv2.threshold(img_H, 140, 255, cv2.THRESH_BINARY)
    cv2.imwrite(now + '_4_' + file_name + '_mask_A.png', img_mask)
    ...
    cv2.imwrite(now + '_5_' + file_name + '_boundingbox_A.png', img)
    # pandasのSeries型へ一旦変換
...
def draw_contours_B(file_name, img):
    ...
    cv2.imwrite(now + '_1_' + file_name + '_gray_B.png', gray)
    
    ret,th1 = cv2.threshold(gray,200,255,cv2.THRESH_BINARY)
    cv2.imwrite(now + '_2_' + file_name + '_th1_B.png', gray)
    # 輪郭抽出
    ...
    cv2.imwrite(now + '_3_' + file_name + '_boundingbox_B.png', img)
...
def draw_contours_C(file_name, img):
    # 2値化
    gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    cv2.imwrite(now + '_1_' + file_name + '_gray_C.png', gray)

    ret, binary = cv2.threshold(gray, 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)
    cv2.imwrite(now + '_2_' + file_name + '_otsu_C.png', binary)

    kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (5, 5))
    binary = cv2.dilate(binary, kernel)
    cv2.imwrite(now + '_3_' + file_name + '_dilate_C.png', binary)
    # 輪郭抽出
...
def draw_contours_D(file_name, img):
    ...
    cv2.imwrite(now + '_' + file_name + '_boundingbox_D.png', edges)
...
if __name__ == '__main__':
    my_png_list = glob.glob("*.png")
    for my_file in my_png_list:
